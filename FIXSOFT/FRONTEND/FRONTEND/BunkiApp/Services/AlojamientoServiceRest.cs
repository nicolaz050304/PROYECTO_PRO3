...
namespace BunkiApp.Services
{
    ...
    public class AlojamientoServiceRest
    {
        ...
        // Moderación (admin): alojamientos en espera de aprobación (estadoValidacion=PENDIENTE).
        public async Task<List<Alojamiento>> ListarPendientesAsync()
            => await _http.GetFromJsonAsync<List<Alojamiento>>($"{Endpoint}/pendientes") ?? new();

        // Moderación (admin): aprueba o rechaza un alojamiento. estado = "APROBADO" | "RECHAZADO".
        public async Task ValidarAsync(int id, string estado)
            => (await _http.PutAsJsonAsync($"{Endpoint}/{id}/validacion", new { estado }))
