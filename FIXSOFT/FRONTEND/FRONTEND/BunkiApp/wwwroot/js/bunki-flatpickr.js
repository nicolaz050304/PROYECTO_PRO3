...
window.bunkiDatePicker = {
    ...
    init: function (selector, options) {
        ...
        flatpickr(selector, {
            ...
            allowInput: false,
            position: "below",
            // Mes como texto estático con flechas (evita el <select> nativo, cuyo dropdown
            // muestra el acento azul del SO y no se puede tematizar con CSS).
    ...
    // Usado por el modal "Bloquear fechas" de Mis alojamientos (RF30): UN calendario en modo
    // rango (rangePlugin) que alimenta dos inputs (inicio/fin). Avisa a Blazor del rango elegido
    // vía SetBloqueoRango. Permite deshabilitar fechas ya ocupadas/bloqueadas (options.disable).
    initBloqueoRango: function (selectorEntrada, selectorSalida, options, dotNetRef) {
        if (typeof flatpickr === "undefined") {
            console.error("flatpickr no está cargado.");
            return;
        }
        ...
        // Destruimos cualquier instancia previa sobre estos inputs: el modal se reinicializa
        // al abrirse y tras cada alta/baja de bloqueo, y no queremos calendarios apilados.
        [selectorEntrada, selectorSalida].forEach(function (sel) {
        ...
        var config = {
            ...
            onChange: function (selectedDates) {
                var iso = function (d) { return d ? formatearISO(d) : null; };
                if (selectedDates.length >= 2) {
                    dotNetRef.invokeMethodAsync("SetBloqueoRango", iso(selectedDates[0]), iso(selectedDates[1]));
                } else if (selectedDates.length === 1) {
                    dotNetRef.invokeMethodAsync("SetBloqueoRango", iso(selectedDates[0]), null);
                } else {
                    dotNetRef.invokeMethodAsync("SetBloqueoRango", null, null);
                }
            }
        };
        ...
        flatpickr(selectorEntrada, config);
    },
