...
public class Alojamiento
{
    ...
    [JsonPropertyName("estadoValidacion")] public string EstadoValidacion { get; set; } = "PENDIENTE";
    [JsonPropertyName("servicios")] public List<string> Servicios { get; set; } = new();
    ...
    [JsonPropertyName("reglas")] public List<string> Reglas { get; set; } = new();
...
public class Resena
{
    ...
    [JsonPropertyName("autorId")] public int AutorId { get; set; }
    ...
    public string Objetivo { get; set; } = "ALOJAMIENTO";
    // Anfitrión (dueño del alojamiento). Para reseñas Objetivo="ANFITRION".
...
public class Usuario
{
    ...
    // Validación documentaria (RF02): PENDIENTE / APROBADO / RECHAZADO. Real del backend.
    ...
    // Identidad verificada por un admin (RF02). Es el gate para publicar alojamientos.
    [JsonIgnore]
    public bool IdentidadVerificada =>
        string.Equals(EstadoValidacion, "APROBADO", StringComparison.OrdinalIgnoreCase);
...
public class RecuperarPasswordForm
{
    ...
    // Validación de identidad sin correo (el sistema no envía emails): número de documento (DNI/Pasaporte).
...
public class BusquedaFiltros
{
    ...
    // RF08: filtro por país. "" = todos.
    public string Pais { get; set; } = "";
    // Filtro por servicios/amenidades (el alojamiento debe tener TODOS los seleccionados).
...
public class CuentaBancaria
{
    ...
    // RF15: cuenta de cobro principal (recibe el abono automático de las reservas confirmadas).
    public bool Principal { get; set; }
}

// Movimiento del estado de cuenta (RF15): depósito, retiro o abono de reserva.
public class MovimientoCuenta
{
    public int Id { get; set; }
    public int IdCuenta { get; set; }
    public string Tipo { get; set; } = "";   // DEPOSITO | RETIRO | ABONO
    public double Monto { get; set; }
    public string? Descripcion { get; set; }
    public double SaldoResultante { get; set; }
    public string? Fecha { get; set; }

    // Ingreso = suma al saldo (DEPOSITO/ABONO). Egreso = resta (RETIRO/REEMBOLSO).
    [System.Text.Json.Serialization.JsonIgnore]
    public bool EsIngreso =>
        string.Equals(Tipo, "DEPOSITO", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(Tipo, "ABONO", StringComparison.OrdinalIgnoreCase);
    [System.Text.Json.Serialization.JsonIgnore]
    public string TipoTexto => Tipo switch
    {
        "DEPOSITO" => "Depósito",
        "RETIRO" => "Retiro",
        "ABONO" => "Abono de reserva",
        "REEMBOLSO" => "Reembolso (cancelación)",
        _ => Tipo
    };
}
