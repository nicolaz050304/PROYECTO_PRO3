...
public class AlojamientoImpl implements AlojamientoIDAO {
    ...
    private static volatile boolean columnasListas = false;
    ...
    /**
     * La tabla `alojamiento` ya existía sin columnas de servicios/reglas. Las agregamos en caliente
     * (una sola vez) si faltan — MySQL no soporta ADD COLUMN IF NOT EXISTS, así que consultamos
     * information_schema. Se guardan como CSV en columnas TEXT.
     */
    private static synchronized void asegurarColumnas() {
        ...
        try (Connection c = DBManager.getInstance().getConnection();
            ...
            for (String col : new String[]{"servicios", "reglas"}) {
                ...
                try (ResultSet rs = st.executeQuery(check)) {
                    ...
                    if (!existe) st.executeUpdate("ALTER TABLE alojamiento ADD COLUMN " + col + " TEXT NULL");
                }
            }
            columnasListas = true;
    ...
    /** Lista -> CSV (null si vacía, para no guardar cadenas vacías). */
    private static String toCsv(List<String> xs) {
        return (xs == null || xs.isEmpty()) ? null : String.join(",", xs);
    }

    /** CSV -> lista (tolera null/espacios). */
    private static List<String> fromCsv(String s) {
